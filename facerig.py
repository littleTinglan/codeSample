...
def UI():
    ...
    mc.text(label='Align all joints with your character')
    mc.button(label='Step 2: Create Joints', command = 'createJnt()')
    mc.button(label='Step 3: Create Rig', command = 'createRig()')
    mc.text(label='Step 4: Bind face')
    mc.button(label='Step 5: Attach blend shapes', command = 'attachBlendShapes()')
...
def createRig():
	...
	#createController('Mouth', charName, 'center')
	#mc.setAttr(charName + '_Mouth_limitBox.translateX', 10)
	#mc.setAttr(charName + '_Mouth_limitBox.translateY', 5)
	#mc.select(charName + '_Mouth_ctrl')
	#mc.addAttr( shortName = 'L_in_out', longName = 'LeftCornerInOut', defaultValue = 0.0, minValue = -1.0, maxValue = 1, hidden = False, keyable = True)
	#mc.addAttr( shortName = 'L_Up_Down', longName = 'LeftCornerUpDown', defaultValue = 0.0, minValue = -1.0, maxValue = 1, hidden = False, keyable = True)
	#mc.addAttr( shortName = 'R_in_out', longName = 'RightCornerInOut', defaultValue = 0.0, minValue = -1.0, maxValue = 1, hidden = False, keyable = True)
	#mc.addAttr( shortName = 'R_Up_Down', longName = 'RightCornerUpDown', defaultValue = 0.0, minValue = -1.0, maxValue = 1, hidden = False, keyable = True)
	#createController('Tongue', charName, 'center')
	#mc.setAttr(charName + '_Tongue_limitBox.translateX',10)
	#mc.setAttr(charName + '_Tongue_limitBox.translateY',2.8)
	#mc.setAttr(charName + '_Tongue_limitBox.scaleX', .5)
	#mc.setAttr(charName + '_Tongue_limitBox.scaleY', .5)
	#mc.setAttr(charName + '_Tongue_limitBox.scaleZ', .5)
	...
	#mc.setDrivenKeyframe('c_'+charName+'_jaw_joint_bind.rotateZ', currentDriver = charName+'_Mouth_ctrl.translateY', driverValue=0, value=0)
	#mc.setDrivenKeyframe('c_'+charName+'_jaw_joint_bind.rotateZ', currentDriver = charName+'_Mouth_ctrl.translateY', driverValue=-1, value=-45)
...
def attachBlendShapes():
	...
	for bs in listBlendShapes:
		...
		if geoName == 'L_Brow*':
			...
			elif geoName == 'L_Brow_InIn':
				mc.setDrivenKeyframe('faceBlendShapes.L_Brow_InIn', currentDriver = charName + '_L_Brow_ctrl.LeftInInOut', driverValue = 0, value = 0)
				mc.setDrivenKeyframe('faceBlendShapes.L_Brow_InIn', currentDriver = charName + '_L_Brow_ctrl.LeftInInOut', driverValue = -1, value = 1)
				mc.setDrivenKeyframe('faceBlendShapes.L_Brow_InIn', currentDriver = charName + '_L_Brow_ctrl.translateX', driverValue = 0, value = 0)
				mc.setDrivenKeyframe('faceBlendShapes.L_Brow_InIn', currentDriver = charName + '_L_Brow_ctrl.translateX', driverValue = -1, value = 1)
			else:
				mc.addAttr(longName = geoName, defaultValue = 0.0, minValue = 0.0, maxValue = 1.0, hidden = False, keyable = True)
		if geoName == 'R_Brow*':
		...
		mc.select(charName + '_R_Eye_Blink')
		...
		mc.select(charName + '_L_Eye_Blink')
